...
export default function Node(props){
    ...
    const { isStartPoint, isEndPoint, isWall, isVisited, id, row, col } = props.data

    const { isMouseDown, updateNodes, selectedNode } = useContext(PathFinderContext)

    function getNodeClasses(){
        return `node${isStartPoint?' start-node':''}${isEndPoint?' end-node':''}${isWall?' wall-node':''}${isVisited?' visited-node':''}`
    }
